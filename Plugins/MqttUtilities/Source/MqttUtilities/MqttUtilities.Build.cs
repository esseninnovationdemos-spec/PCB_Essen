...
public class MqttUtilities : ModuleRules
{
    ...
    public void LoadThirdPartyLibrary(string libname, ReadOnlyTargetRules Target)
    {
        ...
        // Windows: Link the static .lib file
        if(Target.Platform == UnrealTargetPlatform.Win64)
        {
            PublicAdditionalLibraries.Add(Path.Combine(LibrariesPath, libname + StaticLibExtension));
        }

        // Linux: Link the .so file (Dynamic) instead of the .a file (Static)
        // This fixes the "relocation R_X86_64_PC32" error
        if(Target.Platform == UnrealTargetPlatform.Linux)
        {
            PublicAdditionalLibraries.Add(Path.Combine(LibrariesPath, libname + DynamicLibExtension));
        }
        ...
        // We use the DynamicLibExtension for copying on all platforms (dll, dylib, so)
